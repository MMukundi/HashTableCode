...
class QuadraticHashTable<K, V> extends HashTableBase<K, V>
{
    ...
    // The number the probing index is multiplied by, ie if QUADRATIC_COEFFICIENT=2, g(i)=2(i^2)+h, where h is the hash value
    private static final int QUADRATIC_COEFFICIENT = 1;

    // The number the probing index squared is added to by, ie if QUADRATIC_CONSTANT=3, g(i)=i^2+h+3, where h is the hash value
    private static final int QUADRATIC_CONSTANT = 0;

    // The final hash then:
    // g(x) = (h+LINEAR_COEFFICIENT*(x^2)+LINEAR_CONSTANT)%TABLE_SIZE
